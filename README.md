...
### Отдельные endpoint'ы по одному файлу

Если внешняя система плохо работает с `multipart`, можно вызывать отдельные маршруты, каждый из которых возвращает только один файл:

- `POST /process1` — `crop_preview.jpg`
- `POST /process2` — `improved.jpg`
- `POST /process3` — `bw.png`
- `POST /process4` — `high_contrast.jpg`
- `POST /process5` — `debug_roi.jpg`

У них те же параметры, что и у `/process`:
...
```bash
curl -X POST "http://localhost:8000/process1?detector_backend=craft" \
  -F "file=@/path/to/bottle.jpg" \
  --output crop_preview.jpg
```
